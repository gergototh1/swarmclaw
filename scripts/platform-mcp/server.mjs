...
/**
 * SwarmClaw's OWN platform tools, as a stdio MCP server.
 *
 * WHY. An agent on a CLI provider runs its own tool loop and never receives the
 * LangChain array the host assembles; the platform's docs say so
 * (`/docs/providers`: CLI providers "manage their own runtime/tool loop"). The
 * extensions each solved that for themselves by fronting their tools as MCP
 * servers. The platform's own tools -- `manage_tasks`, `spawn_subagent`,
 * `manage_agents` and the rest -- had no such front, which is why an agent on
 * `claude-cli` could not hand work to another agent however many capabilities
 * the operator ticked for it. This file is that front.
 *
 * WHAT IT IS NOT. It runs nothing itself. Every call goes to the host over
 * `POST /api/platform-mcp`, which builds the calling agent's REAL tool array
 * with the same `buildSessionTools()` a normal turn uses and then runs the one
 * that was asked for. So the capability selection, the capability policy and
 * the `delegationEnabled` gate all apply here exactly as they do elsewhere: a
 * tool the agent would not have been given is not offered, and cannot be run.
 *
 * IT NAMES NO TOOL. `tools/list` asks the host what THIS agent has, so two
 * agents pointed at this same server see different tables, and a capability the
 * operator grants tomorrow appears with no change here.
 *
 * It is spawned by the CLI with plain `node`, so it has no dependency and
 * imports nothing from the host. The port-file contract it re-implements is the
 * one in src/lib/server/runtime/port-file.ts.
 *
 * FINDING THE HOST. The host's port differs between launches, so it writes
 * `run/port.json` at boot and this shim reads it from `SWARMCLAW_PORT_FILE`.
 * There is no fallback port: a shim that guessed 3456 would, on a machine where
 * another program owns 3456, hand an agent's requests to that program. The
 * file has to pass all four checks of the contract -- shape, boot time and pid,
 * the `/api/healthz` service name, and the instance token -- and it is read
 * fresh on every call, because the host can restart on another port while this
 * process lives.
 *
 ...
 * it writes the per-turn MCP config (`addAssignedMcpServers`,
 * src/lib/providers/claude-cli.ts). Here that stamp is load-bearing in a way it
 * is not for an extension shim: it decides WHOSE capability set is built. The
 * agent never sees these values and cannot name itself, and an `agentId` passed
 * as a tool argument stays an argument. Do not ever read the caller from
 * `params.arguments`.
 */
 ...
 * with its status.
 */
async function callHost(op, body, timeoutMs) {
  ...
  let res
  try {
    res = await fetch(`${target.base}/api/platform-mcp`, {
      method: 'POST',
      headers,
    ...
    if (err && err.name === 'TimeoutError') {
      return { error: { code: 'host_hiba', reason: 'idotullepes', message: `a host ${timeoutMs} ms alatt nem felelt a(z) ${op} kérésre; a kérés a hostban még futhat` } }
    }
  ...
  if (res.status === 404) {
    return { error: { code: 'platform_mcp_hianyzik', message: `a hoston nincs /api/platform-mcp útvonal: a futó kiadás régebbi, mint ez a shim (${hostMessage(json, 404)})` } }
  }
  if (res.status === 429) {
...
async function listTools() {
  const answer = await callHost('tools', callerStamp(), LIST_TIMEOUT_MS)
...
async function callTool(id, params) {
  ...
  const answer = await callHost('call', { tool: name, args, ...callerStamp() }, CALL_TIMEOUT_MS)
  // The route wraps a tool answer as { result }; a refusal arrives as { error }.
  return reply(id, toolResult(isPlainObject(answer) && 'result' in answer ? answer.result : answer))
}
