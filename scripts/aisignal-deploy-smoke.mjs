...
/**
 * Runs the AI Signal deploy smoke against the BUILT server, from a checkout.
 *
 * `extensions/aisignal/test/deploy.smoke.mjs` takes a server that is already
...
const EXT_ROOT = path.join(REPO_ROOT, 'extensions', 'aisignal')
const STANDALONE_SERVER = path.join(REPO_ROOT, '.next', 'standalone', 'server.js')
const HEALTH_TIMEOUT_MS = 120_000

function log(message) {
  console.log(`[aisignal deploy-local] ${message}`)
}
...
async function main() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'swarmclaw-aisignal-deploy-'))
...
main().catch((err) => {
  console.error(`aisignal deploy smoke FAILED: ${err instanceof Error ? err.message : String(err)}`)
  process.exit(1)
})
