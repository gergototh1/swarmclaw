...
/**
 * Runs the video extension deploy smoke against the BUILT server, from a checkout.
 *
 * `extensions/video/test/deploy.smoke.mjs` takes a server that is already
...
const EXT_ROOT = path.join(REPO_ROOT, 'extensions', 'video')
const STANDALONE_SERVER = path.join(REPO_ROOT, '.next', 'standalone', 'server.js')
const HEALTH_TIMEOUT_MS = 120_000

function log(message) {
  console.log(`[video deploy-local] ${message}`)
}
...
async function main() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'swarmclaw-video-deploy-'))
