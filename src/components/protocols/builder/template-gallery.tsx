  ...
  const renderCard = (template: ProtocolTemplate) => (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full border border-line-default bg-layer-2 px-2 py-1 text-[10px] font-700 tracking-[0.03em] text-text-3">
  ...
  return (
    <div className="space-y-4">
      {builtInTemplates.length > 0 && (
        <div>
          <h4 className="mb-2 text-[11px] font-700 tracking-[0.03em] text-text-3">Built-in</h4>
          <div className="grid gap-3 md:grid-cols-2">{builtInTemplates.map(renderCard)}</div>
        </div>
      )}
      {customTemplates.length > 0 && (
        <div>
          <h4 className="mb-2 text-[11px] font-700 tracking-[0.03em] text-text-3">Custom</h4>
