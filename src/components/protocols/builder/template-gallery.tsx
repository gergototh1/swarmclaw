...
export function TemplateGallery({ templates: providedTemplates }: Props) {
  ...
  const renderCard = (template: ProtocolTemplate) => (
    <button
      ...
      className="rounded-[16px] border border-line-subtle bg-layer-1 p-4 text-left transition-all hover:border-accent-bright/20 hover:bg-layer-2 cursor-pointer"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="text-[14px] font-display font-700 text-text">{template.name}</div>
        <span className="rounded-full border border-line-default bg-layer-2 px-2 py-1 text-[10px] font-700 uppercase tracking-[0.12em] text-text-3/70">
