...
function CommandPaletteInner({ setOpen }: { setOpen: (v: boolean) => void }) {
  ...
  const items = useMemo<CommandItem[]>(() => {
    ...
    const views = [
      ...
      { id: 'vault', label: 'Vault', description: 'API keys, encrypted secrets, and agent wallets', keywords: ['api key', 'token', 'credential', 'secret', 'wallet'] },
      { id: 'stream', label: 'Stream', description: 'Run history, entity audit trail, and application logs', keywords: ['runs', 'activity', 'logs', 'audit', 'history'] },
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 text-left border-none cursor-pointer transition-all relative z-10
                        ${idx === selectedIndex ? 'bg-accent-soft text-accent-bright pl-6 scale-[1.01]' : 'bg-transparent text-text-2 hover:bg-layer-2'}`}
