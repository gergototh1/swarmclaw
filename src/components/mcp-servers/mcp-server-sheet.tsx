  ...
  return (
    <>
            ...
            {MCP_PRESETS.map((preset) => {
              ...
              return (
                <button
                  ...
                  className={`py-2 px-4 rounded-md border text-[13px] font-600 cursor-pointer transition-all ${
                    isActive
                    ...
                    <div className="space-y-1 rounded-md border border-line-default bg-surface/50 p-2 max-h-[320px] overflow-auto">
                      ...
                      {discoveredTools.map((t) => {
                        ...
                        return (
                          <label
                            key={t.name}
                            className={`flex items-start gap-3 p-2 rounded-sm cursor-pointer transition-colors ${checked ? 'bg-accent-bright/5' : 'hover:bg-layer-1'}`}
                          >
                            <input
                              type="checkbox"
                              checked={checked}
      ...
      {editing && (
        <div className="mb-8">
          <button
            onClick={handleTest}
            disabled={testing}
            className="py-3 px-6 rounded-md border border-line-default bg-transparent text-text-2 text-[14px] font-600 cursor-pointer hover:bg-surface-2 transition-all disabled:opacity-30"
            style={{ fontFamily: 'inherit' }}
          >
            {testing ? 'Testing...' : 'Test Connection'}
          </button>
          {testResult && (
            <div className={`mt-3 p-3 rounded-sm text-[13px] ${testResult.ok ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
              {testResult.ok ? (
                <>
      ...
      <div className="flex gap-3 pt-2 border-t border-line-subtle">
        ...
        <button onClick={handleSave} disabled={!canSave} className="flex-1 py-3.5 rounded-md border-none bg-accent-bright text-accent-fg text-[15px] font-600 cursor-pointer active:scale-[0.97] disabled:opacity-30 transition-all shadow-[0_4px_20px_rgba(99,102,241,0.25)] hover:brightness-110" style={{ fontFamily: 'inherit' }}>
          {editing ? 'Save' : 'Create'}
        </button>
      </div>
      <ConfirmDialog
