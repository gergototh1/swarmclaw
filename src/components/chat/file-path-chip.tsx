  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-sm bg-layer-2 border border-line-default font-mono text-[13px]">
      ...
      {canServe && serverState.running && (
        <>
          {frameworkLabel && (
            <span className="px-1.5 py-0.5 rounded-xs bg-indigo-500/15 text-indigo-300 text-[9px] font-700 uppercase tracking-wider">
              {frameworkLabel}
            </span>
          )}
          {serverState.type === 'npm' && (
            <span className="px-1.5 py-0.5 rounded-xs bg-amber-500/15 text-amber-300 text-[9px] font-700 uppercase tracking-wider">
              npm
            </span>
          )}
          <a
