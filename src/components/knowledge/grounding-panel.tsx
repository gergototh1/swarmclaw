...
export function GroundingPanel(props: {
  ...
  return (
    <details className={`group rounded-lg border border-sky-400/15 bg-sky-400/[0.04] ${props.className || ''}`}>
      <summary className="flex cursor-pointer list-none items-center gap-2 px-3.5 py-2.5 select-none [&::-webkit-details-marker]:hidden">
        ...
        <span className="text-[11px] font-600 tracking-[0.03em] text-sky-200/80">{title}</span>
      ...
      <div className="space-y-2 px-3.5 pb-3 pt-1">
        {props.retrievalTrace?.query && (
          <div className="rounded-md border border-line-subtle bg-layer-2 px-3 py-2 text-[11px] text-text-3">
            Query: <span className="text-text-2">{props.retrievalTrace.query}</span>
          </div>
        )}

        {items.map((citation) => (
          <div key={`${citation.sourceId}:${citation.chunkId}`} className="rounded-md border border-line-subtle bg-layer-2 px-3 py-2.5">
