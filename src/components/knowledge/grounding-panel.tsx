    ...
    <details className={`group rounded-lg border border-sky-400/15 bg-sky-400/[0.04] ${props.className || ''}`}>
      ...
      <div className="space-y-2 px-3.5 pb-3 pt-1">
        {props.retrievalTrace?.query && (
          <div className="rounded-sm border border-line-subtle bg-layer-2 px-3 py-2 text-[11px] text-text-3">
            Query: <span className="text-text-2">{props.retrievalTrace.query}</span>
          </div>
        )}

        {items.map((citation) => (
          <div key={`${citation.sourceId}:${citation.chunkId}`} className="rounded-sm border border-line-subtle bg-layer-2 px-3 py-2.5">
