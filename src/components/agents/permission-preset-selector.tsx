...
export function PermissionPresetSelector({ agentId, onPresetChanged }: Props) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="block text-[11px] font-600 tracking-[0.03em] text-text-3">Permission Preset</label>
      <div className="flex gap-2">
        {PRESETS.map((p) => (
          <button
            ...
            disabled={applying}
            className={`flex-1 flex flex-col items-center gap-1 py-2.5 px-2 rounded-sm border cursor-pointer transition-all
              ${current === p.id
                ? p.color
                : 'bg-layer-1 border-line-subtle text-text-3 hover:border-line-default'
              } ${applying ? 'opacity-50' : ''}`}
