    ...
    <main className="min-h-screen bg-bg text-text">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-5 py-8 sm:px-8 lg:px-10">
        <header className="border-b border-line-subtle pb-6">
          <div className="text-[11px] font-700 tracking-[0.03em] text-emerald-300/80">
        ...
        {renderBody(payload)}
        <footer className="border-t border-line-subtle pt-4 text-[11px] text-white/45">
...
function renderReportLine(line: string, index: number) {
  ...
  if (trimmed === '---') return <hr key={index} className="my-4 border-line-subtle" />
  if (trimmed.startsWith('### ')) {
    return <h5 key={index} className="mb-2 mt-4 text-[13px] font-700 text-white">{trimmed.slice(4)}</h5>
  }
  if (trimmed.startsWith('## ')) {
    return <h4 key={index} className="mb-2 mt-5 text-[13px] font-700 tracking-[0.03em] text-white/70">{trimmed.slice(3)}</h4>
  }
  if (trimmed.startsWith('# ')) {
      ...
      <section className="flex flex-col gap-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <div className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-700 tracking-[0.03em] ${statusTone}`}>
        ...
        {payload.latestReport ? (
          <article className="rounded-lg border border-emerald-400/20 bg-emerald-400/[0.04] p-4">
      ...
      <section className="section-band -line-subtle">
        ...
        {payload.tags.length > 0 ? (
          <div className="mt-2 flex flex-wrap gap-2">
            {payload.tags.map((t) => (
              <span key={t} className="rounded-xs bg-layer-2 px-2 py-0.5 text-xs text-white/70">
                {t}
              </span>
            ))}
          </div>
