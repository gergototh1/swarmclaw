...
- Managed resources let an extension declare provisionable agents, schedules/routines, trusted local folders, gateway platforms, and setup checks. Nothing reconciles them on install, enable or upgrade: an operator presses Reconcile on the extension's card in Extensions, runs "swarmclaw extensions reconcile --extension-id <file>", or posts to /api/extensions/managed-resources. Until then the declared agents and routines do not exist, so say so in your extension's own UI rather than letting it look idle.
