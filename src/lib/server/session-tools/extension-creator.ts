...
- Managed resources let an extension declare provisionable agents, schedules/routines, trusted local folders, gateway platforms, and setup checks. The host reconciles the declared agents and routines on install, on enable and on upgrade, and an operator can ask again with Reconcile on the extension's card in Extensions, "swarmclaw extensions reconcile --extension-id <file>", or a post to /api/extensions/managed-resources. That reconcile can still fail or skip a declaration, and only agents and routines are created this way - local folders, gateway platforms and setup checks are configured by the operator - so check the resources exist before acting on them and say what is missing in your extension's own UI rather than letting it look idle.
