...
test('mcpCall builds the session shape a tool expects from the stamped caller', async () => {
  const { mcpCall } = createMcpBridge(tools)
  const out = await mcpCall({ tool: 'echo', args: {}, agentId: 'a1', agentName: 'GTassistant', sessionId: 's1' })
  ...
  assert.deepEqual(out.ctx.session.agentRecord, { id: 'a1', name: 'GTassistant' })
})
...
/**
 * The docs extension is the reason the stamp carries a NAME and not only an id:
 * every agent gets its own folder, and the folder is called after the name.
 * Without it the slug falls back to the head of the id and an operator opening
 * the vault in Finder sees `agents/c3377d/` where `agents/gtassistant/` was the
 * entire point.
 */
test('the agent name survives the trip, because the folder is named after it', async () => {
  const seen = []
