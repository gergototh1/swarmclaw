...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aisignal-mcp-'))
let fileNo = 0
...
const TOOL_TABLE = { tools: [{ name: 'signalSweep', description: 'sweep', inputSchema: { type: 'object', properties: {} } }] }
...
test('tools/list comes from the host, so the extension stays the only place a tool is defined', async () => {
  const host = await fakeHost(({ url }) => (
    url === '/api/extensions/aisignal.mjs/call/mcpTools' ? { status: 200, json: TOOL_TABLE } : { status: 404, json: {} }
  ))
  ...
  try {
    ...
    assert.equal(init.result.serverInfo.name, 'swarmclaw-aisignal')
    const list = await shim.call('tools/list', {})
    assert.deepEqual(list.result.tools.map((t) => t.name), ['signalSweep'])
...
test('tools/call forwards the caller the host stamped into the env, never an argument', async () => {
  ...
  const shim = startShim({
    ...
    SWARMCLAW_AGENT_NAME: 'Signal Scout',
    SWARMCLAW_SESSION_ID: 'sess-9',
  })
  try {
    const r = await shim.call('tools/call', { name: 'signalSweep', arguments: { agentId: 'HAZUDIK' } })
    const sent = JSON.parse(r.result.content[0].text).got
    assert.equal(sent.agentId, 'agent-7')
    assert.equal(sent.agentName, 'Signal Scout')
...
test('an unstamped shim sends no caller at all rather than an empty one', async () => {
  ...
  try {
    const r = await shim.call('tools/call', { name: 'signalSweep', arguments: {} })
    const sent = JSON.parse(r.result.content[0].text).got
...
test('a tool error from the host comes back as isError, and a value comes back as a value', async () => {
  ...
  try {
    const bad = await shim.call('tools/call', { name: 'signalSweep', arguments: { rossz: true } })
    assert.equal(bad.result.isError, true)
    const good = await shim.call('tools/call', { name: 'signalSweep', arguments: {} })
...
test('a bad tool name and an unknown method are refused by their JSON-RPC code, with no call sent', async () => {
  ...
  try {
    ...
    assert.equal((await shim.call('tools/call', { name: 'signalSweep', arguments: 'nem objektum' })).error.code, -32602)
