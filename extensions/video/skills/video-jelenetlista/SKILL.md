...
**A forma:** `cimlap` → tartalom → záró `allitas`. Az első jelenet mindig
`cimlap`, az utolsó mindig `allitas`, legalább 3 és legfeljebb 12 jelenet.
Nincs `cta`: a felszólítás a záró `allitas` `mondat`-ában van.
...
propok mind. A `hang` és a `lathatoHossz` soha: azokat a modul írja a mért
narrációból. Kép csak létező fájl a `public/` alatt, `/`-jellel, `..`
nélkül.
