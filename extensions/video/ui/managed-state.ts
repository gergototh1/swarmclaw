 ...
 * store, which extension code cannot read. The two are told apart here by the
 * same rule the module is built on: a question that was answered "none" and a
 * question that could not be put are different facts, so the failure of this
 * request is its own state and never reads as "not scheduled".
 *
