...
test('mcpCall builds the session shape a tool expects from the stamped caller', async () => {
  const { mcpCall } = createMcpBridge(tools)
  const out = await mcpCall({ tool: 'echo', args: {}, agentId: 'a1', agentName: 'Videó Lektor', sessionId: 's1' })
  ...
  assert.deepEqual(out.ctx.session.agentRecord, { id: 'a1', name: 'Videó Lektor' })
})
...
test('every tool the extension declares is reachable over the bridge', () => {
  const listed = video.rpc.mcpTools().tools.map((t) => t.name)
  assert.deepEqual(listed, video.tools.map((t) => t.name))
  assert.ok(listed.includes('videoQueue'), 'the queue tool is the one the daily chain runs')
})
