...
export function hostOf(): SwarmclawHostView {
  ...
  if (!host || typeof host.registerPage !== 'function' || !host.modules) {
    throw new Error('tts: window.swarmclaw is not installed; this bundle must be loaded by the SwarmClaw shell')
  }
  return host
}
...
export function hostReact(): unknown {
  ...
  if (!react) throw new Error('tts: host module missing: react')
  return react
}
 ...
 *
 * The registry keys pages on the id of the extension file (`tts.mjs`, not
 * `tts`), refuses a registration whose id differs from the tag it injected,
