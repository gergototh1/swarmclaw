 ...
 *   2. `fiokotOsszekot` -- the write half of connecting a platform account.
 ...
 * and `extensions/video/src/rpc.mjs`'s reads use. `jovahagy`, `fiokotOsszekot`
 * and `atutemez` are buttons the operator presses in states this module
 * deliberately refuses (a release not yet lektorált, no configured slot, a
 * malformed platform id), and refusing THOSE is not a bug -- it is the
  ...
  return {
    ...
    async jovahagy(body = {}) {
      return lever(state, async () => {
        ...
        if (k.allapot !== KIADAS_ALLAPOTOK.LEKTORALT) {
          refuse('kiadas_nincs_lektoralva', `csak lektorált kiadás hagyható jóvá (jelenlegi állapot: ${k.allapot})`, { allapot: k.allapot })
        }
        repo().kiadastJovahagy(kiadasId)
        let utemezve = false
