...
- Produces:
  ...
  - `tabCommandForKey(e: KeyLike, platform: 'desktop-app' | 'browser'): TabCommand | null`
...
**Interfaces:**
- Consumes: Task 2's `parseHostMessage`, `tabCommandForKey`, `appUrlFromHref`, `FrameMessage`, `detectShellMode`, `tabIdFromWindow`, `runTabFlushHandlers`.
...
**Interfaces:**
- Consumes: Task 1 (all of `tabs.ts`), Task 2 (`parseFrameMessage`, `parseTabCommand`, `tabCommandForKey`, `appUrlFromHref`, `TAB_WINDOW_NAME_PREFIX`, `TabCommand`, `HostMessage`, `tabLabel`, `setTabNavigator`, `routeLinkClick`, `navigateInActiveTab`), Task 4 (frames answer `ready`, `location`, `title`, `open-tab`, `command`, `auth-required`, `flushed`).
...
import {
  TAB_WINDOW_NAME_PREFIX, appUrlFromHref, parseFrameMessage, parseTabCommand, tabCommandForKey,
...
export function TabHost() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      const command = tabCommandForKey(e, platform)
