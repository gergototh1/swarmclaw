...
`required: false`: a CRM-1 **működik** nélküle, és egy kötelezőnek jelölt, de
valójában opcionális előfeltétel arra tanítja meg az operátort, hogy a piros
jelzést figyelmen kívül hagyja.

**A Google OAuth-ellenőrzés szándékosan NINCS itt**, pedig kézenfekvő lenne.
Két oka van, és a második a fontosabb. A CRM-1 egyáltalán nem hív Google-t --
a naptár a CRM-4-é --, tehát egy itteni jelzés olyasmiről szólna, amit ez a
fázis nem használ. És a hoszt a kliens-azonosítót deploy-módtól függően
`GOOGLE_OAUTH_CLIENT_DESKTOP_ID`-ból vagy `GOOGLE_OAUTH_CLIENT_WEB_ID`-ból
olvassa (`resolveGoogleClientOrNull`, `src/lib/server/oauth/google.ts`), tehát
egy `kind: 'env'` ellenőrzés egyetlen `target`-tel a két mód egyikében mindig
hamisan jelezne. A helyes kérdés a `ctx.oauth.googleClientConfigured()`, és az
a CRM-4 dolga.
