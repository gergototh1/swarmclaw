...
**Interfaces:**
- Produces: `state` (a `index.mjs` modul-szintű objektuma: `storage`, `settings`, `log`, `oauth`, `repo`); `newId(prefix)` a `src/ids.mjs`-ből.
...
## Task 9: A lap — Ügyfelek lista és Ügyfél lap
...
**Előbb tedd vissza a propokat.** A 3. feladat csonkja szándékosan prop nélküli
`CrmPage()`-et hagyott, mert egy nem használt `rpc` új lint-figyelmeztetés lett
volna, a baseline pedig kiadási kapu. Most van mi használja, tehát itt születik
vissza:
...
és az állomány alján a regisztráció ismét adja át őket:
...
a `makeRpc` importjával együtt (`import { makeRpc, type Rpc } from './api'`), és
a `const rpc = makeRpc(extensionId)` sorral a regisztráció előtt.

Ezután cseréld az `ügyfelek` ágat:
...
## Task 10: Ügyek, Ma nézet, beállítások és a telepítés-ellenőrzés
...
## Task 11: A kézi bevitel befejezése — kapcsolat, cím, ügy

A záró review megállapítása: a lapon ügyfelet fel lehet venni és jegyzetet lehet
írni, de kapcsolatot, email-címet és ügyet nem. Ennek két látható következménye
van, és a második nem halasztott funkció, hanem hiba:

- az ügyfél lap „Kapcsolatok" listája sosem lehet nem üres;
- **az Ügyek nézet négyoszlopos pipeline-ja szerkezetileg elérhetetlen** — nincs
  a kódban út, ami kártyát tehetne bele.

Nyolc rpc-metódus hívó nélkül állt: `createContact`, `attachEmail`,
`createDeal`, `updateDeal`, `updateAccount`, `timeline`, `eventBody`,
`assignUnmatched`. Ez a feladat az elsőt hármat köti be. A `timeline` és az
`eventBody` a lapozásé és a teljes szövegé — az a CRM-2 idővonal-munkájával jön;
az `assignUnmatched` a CRM-2 kapcsolat-választójáé; az `updateDeal` a CRM-3
szakaszléptetéséé.

**Files:**
- Modify: `extensions/crm/ui/ugyfel-lap.tsx` (kapcsolat- és ügy-űrlap)
- Modify: `extensions/crm/ui/style.css`
- Test: `extensions/crm/test/ui.test.mjs`

**Interfaces:**
- Consumes: az rpc `createContact({accountId,name,role,phone})`,
  `attachEmail({contactId,address})`, `createDeal({accountId,kind,title,valueHuf})`
  metódusai (Task 7), és a `mustContact` / `mustAccount` nevesített hibái
- Produces: nincs új exportált felület; a `UgyfelLap` két szakasszal bővül

- [ ] **Step 1: Írd meg a bukó tesztet**

`extensions/crm/test/ui.test.mjs`-hez:
...
Run: `cd extensions/crm && node --import tsx --test test/ui.test.mjs`
Expected: FAIL — `hiányzik a bundle-ből: Új kapcsolat`

- [ ] **Step 3: Kapcsolat-űrlap az ügyfél lapra**

A `Kapcsolatok` szakaszt cseréld erre. A cím hozzáadása **kapcsolatonként külön**
mező, mert egy embernek több címe lehet, és mert az `attachEmail` egy
`contactId`-t vár — ugyanazt a kaput használja, amit a CRM-2 tanult címei:
...
a hozzá tartozó állapottal és két írással:
...
- [ ] **Step 4: Ügy-űrlap az ügyfél lapra**

Az ügyet az ügyfél lapján vesszük fel, nem az Ügyek nézeten: egy ügy mindig
egy ügyfélhez tartozik, és itt már tudjuk, melyikhez. Az Ügyek nézet a
pipeline-t mutatja, nem a felvételt.

A `Nyitott ígéretek` szakasz elé:
...
A `Lap` típus `deals` mezőjét bővítsd a megjelenítéshez szükséges oszlopokkal:
`{ id: string; title: string; stage: string; value_huf: number; closed_at: string | null }`.
...
Run: `npx tsc --noEmit` és `npx eslint extensions/crm/ui/` és `npm run lint:baseline`
Expected: nincs hiba, a baseline 348 marad.
...
git add extensions/crm/ui extensions/crm/test/ui.test.mjs
git commit -F - <<'MSG'
CRM: a kézi bevitel befejezése

Kapcsolat, email-cím és ügy felvétele az ügyfél lapról. Enélkül a
Kapcsolatok lista sosem lehetett nem üres, és az Ügyek nézet
négyoszlopos pipeline-ja szerkezetileg elérhetetlen volt: nem volt út,
ami kártyát tehetett volna bele.

Az ügyet az ügyfél lapján vesszük fel, nem az Ügyek nézeten. Egy ügy
mindig egy ügyfélhez tartozik, és itt már tudjuk, melyikhez; az Ügyek
nézet a pipeline-t mutatja, nem a felvételt.

A cím hozzáadása kapcsolatonként külön mező, mert egy embernek több
címe lehet, és mert ugyanazt az attachEmail kaput hasznalja, amin a
CRM-2 tanult címei is bemennek.
...
| Az MCP-szerver a házon kívüli ügynöknek | CRM-3 |
| A feladatlista az ügyfél lapon (host `/api/tasks`, `customFields.crm_account`) | CRM-3 |
| A javaslat elfogadása → `BoardTask` létrehozása | CRM-3 |
| Idővonal-lapozás (`timeline`) és teljes szöveg (`eventBody`) a lapon | CRM-2 |
| Szakaszléptetés a pipeline-ban (`updateDeal`) | CRM-3 |
...
oldal, kézi bevitel, olvasó tools, a CRM projekt + a
`managedResources.projects` host-kiegészítés." Ezek rendre: Task 4–6 (táblák,
migrációk), Task 9–10 (oldal, kézi bevitel), Task 8 (olvasó tools),
Task 1–3 és 10 (projekt és host-kiegészítés). Hiány nincs.

**Amiért az MCP-szerver kimaradt.** A terv első változata a CRM-1-be tette. A
CRM-1-ben viszont nincs ügynök -- az „Ügyfélkezelő" a CRM-3-ban születik --,
tehát az MCP ajtaja egy üres szobára nyílna, és cserébe a gmail shimjének
mintegy hatszáz sorát kellene átvenni egy fogyasztó nélküli felülethez. A
CRM-3-ban lesz ügynök, és akkor két valódi fogyasztóval dől el az is, hogy a
két shim közös modulba emelendő-e vagy sem -- bizonyíték alapján, nem
találgatásból.

**Amit a terv a specen felül vesz fel, és miért.** A `board` rpc (Task 7) nincs
a specben nevesítve; ez a lap egyetlen alap-lekérdezése, és a nélküli változat
négy külön hívást tenne minden nézetváltásnál.
