...
- **Doksik: markdown documents, shared between you and your agents.** A new extension with its own page in the rail. Documents are real `.md` files under a folder you choose (default `~/SwarmClaw/docs`), so they open in Finder, Obsidian and git; the database beside them is only an index and a full reindex rebuilds it. The page is a WYSIWYG editor over a folder tree, with full-text search that folds Hungarian accents (`komuves` finds `kőműves`), version history with restore, `[[wiki links]]` and backlinks, and a trash. Agents get six tools and a folder of their own: a document written without a folder lands in `agents/<name>/`, an agent may read every folder but write only its own and the shared one, and each turn is told what documents it already has so the tools are used rather than merely available.
- **Editing the same document from both sides is safe.** Every write carries the version it was built on. A save on top of a version somebody else has already replaced is refused rather than applied: you get the other side's text and three buttons, and an agent gets a message telling it to re-read and merge. Nothing is overwritten silently, including edits made outside the app — a document changed in Obsidian is noticed and indexed within seconds.
- **The video kit's scene types are a gallery you can filter.** The Sablonok view shows all twenty-four types with a real Remotion-rendered still of each, cached under the catalogue hash, alongside what each type does and which props it takes. Filters narrow by measured facts only, and a control the kit cannot answer for is disabled and says why.
- **A plain `always: true` in a SKILL.md frontmatter is now honoured.** Before, only the scoped `metadata.openclaw.always` spelling worked and the plain key was silently ignored. The bundled `skills/resourceful-problem-solving/SKILL.md` carries the plain key, so **on this upgrade it enters every agent's prompt on every instance** (about 1.5 k characters per turn), with no migration step and no per-agent scoping -- `always` has none. The same happens to any workspace or project SKILL.md that uses the key. To see which skills are always-on, ask an agent that has `manage_skills` bound for action `status` and read the `always` field (an agent on a scoped `tools` list without it, such as the two AI Signal agents, cannot run that action); or read the frontmatter of the SKILL.md files in the three layers. To turn one off durably, shadow it in the workspace layer: create `<swarmclaw-home>/skills/<name>/SKILL.md` with the same frontmatter `name` and without the `always` key (a copy of the file with that key removed will do). Discovery dedupes on `name` with the workspace layer winning over the bundled one, and the workspace layer is yours, so the shadow survives an update. Editing or deleting the bundled `skills/resourceful-problem-solving/SKILL.md` also works, but only until the next pull or update: that file is read from the app's own tree (`process.cwd()/skills`), which the Docker image and the Electron bundle restore.
